...
- **Qué hicimos:** T-030 (P3 PASS): PoCs + triple LIVE PASS (runners `tools/t030_live_*.py`, F-015–F-024, `docs/T030-POC.md`). T-031 (P4 PASS): dock unificado `src/metadata.*` + `src/metadata_dock.*`, matriz viva T1–T20 en verde, fixes F-025–F-028, CI verde en PR #9 (abierta, sin merge). Limitaciones provisionales heredadas por P5: tokens solo en memoria, Disconnect sin revoke, credenciales por env vars, secret YouTube Desktop/Kick sin distribución (F-017/F-019).
...
- **Deuda conocida:** R1/R2 cerrados (T-013, ADR-007); distribución de secrets Desktop (F-017/F-019) y storage OS, diferidos a P5 (T-032).
