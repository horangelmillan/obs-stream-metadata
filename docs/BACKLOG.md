...
| T-063 | pendiente | 8 | [Commercial] F-C1 cifrado de tokens en reposo (AEAD + Secret Manager + migración 002 sin romper sesiones) | tests + dump ciphertext + deploy dual-read + sonda prod | docs/FASES-COMERCIAL.md |
| T-064 | pendiente | 8 | [Commercial] F-C2 borrado real de datos + privacidad operativa (endpoint, UI, uninstaller, contacto) | conteos DB a cero + OAuth nuevo obligatorio | docs/FASES-COMERCIAL.md |
